...
class FolderPathC{
    ...
    set inputStr(str){
        this.arList = str ? str.split('\\').filter(function(s){return s}) : ['empty path'];
    }

    set color(col){
        col ? this.elem.style.setProperty('background-color',col) : this.elem.style.setProperty('background-color','White');
    }
    ...
    onClick(event){  /* action검사와 그 메소드가 있는지 확인할 것  */
        let fn,
...
function FolderPathF(elem){
    ...
    FolderPathF.prototype.onClick = function(event){  /* action검사와 그 메소드가 있는지 확인할 것  */
        let fn,
            action = event.target.dataset.action;
            action && (fn = this["on"+action]);
            if(fn){
